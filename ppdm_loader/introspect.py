...
def introspect_fk_by_child_col(
    ...
    *,
    ...
    If multiple FKs match the same child column, picks the FK with the most columns.
    """
    child_schema = (child_schema or "").strip()
    child_table = (child_table or "").strip()
    child_col = (child_col or "").strip()
    ...
    for fk_name, g in df.groupby("fk_name"):
        ...
        if n > best_n:
            best_n = n
            best_fk_name = fk_name
            best_g = g.sort_values("ordinal")
...
def introspect_all_fks_for_child_table(conn, *, child_schema: str, child_table: str) -> list[FKInfo]:
    """
    Returns all FKs for a child table, grouped by FK name, with ordered pairs.
    Useful for Step 5 scanning.
    """
    child_schema = (child_schema or "").strip()
    child_table = (child_table or "").strip()
